...
.ease-hover-morph-card {
  will-change: border-radius, transform; /* always on */
}
```

`will-change` hints the browser to promote an element to a GPU compositing layer permanently. Applying it statically means every `.ease-hover-morph-card` element on the page is GPU-promoted even when not animating — consuming GPU memory unnecessarily, especially on pages with many cards.

## Solution

Move `will-change` to the `:hover` state so GPU promotion only happens just before the animation occurs:
